...
source "$ROOT/scripts/verify-functions.private.sh"

dependency_installed "kubectl"

# shellcheck source=gke-istio-shared/verify-db-ratings.sh
# TODO: test integration incorrectly passes when mysql is not installed on vm
source "$ROOT/scripts/verify-db-ratings.private.sh" "$((1 + RANDOM % 5))"
